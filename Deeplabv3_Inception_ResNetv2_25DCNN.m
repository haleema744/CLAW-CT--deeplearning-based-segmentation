%% ========================================================================
%  CLAW SEGMENTATION WITH DEEPLABV3+ (Inception-ResNet-v2 Backbone, 2.5D)
%  ------------------------------------------------------------------------
...
%    2. Build 2.5D inputs (3 consecutive slices stacked as 3 channels).
%    3. For each fold: split data into train / val / test (LOOCV).
%    4. Train DeepLabv3+ with Inception-ResNet-v2 backbone (ImageNet
%       pretrained) with validation monitoring.
...
%                Deep Learning Toolbox Model for Inception-ResNet-v2
%                Network, Computer Vision Toolbox.
%% ========================================================================
...
allX = cell(numClaws, 1);             % Each cell: 2.5D input samples
...
for clawIdx = 1:numClaws
    ...
    % Skip claws that have fewer than 3 slices (2.5D window needs >= 3)
    if numSlices < 3
        fprintf('  x Not enough slices (need at least 3, found %d)\n', numSlices);
        continue;
    end
    ...
    % Skip claw if fewer than 3 slices loaded (2.5D window requirement)
    if loadedCount < 3
        fprintf('  x Only %d slices loaded (need at least 3)\n', loadedCount);
        continue;
    end
...
%% ------------------ 3. PREPARE 2.5D DATA FOR EACH CLAW -----------------------
disp(' ');
disp('========================================');
disp('PREPARING 2.5D DATA FOR ALL CLAWS');
disp('========================================');

% 2.5D window configuration (3 consecutive slices => 3 input channels)
windowSize  = 3;
halfWin     = floor(windowSize/2);         % = 1 for windowSize 3

% Inception-ResNet-v2 native input resolution (must be 299x299)
networkSize = 299;
...
% Loop through each valid claw to build 2.5D samples
for clawIdx = 1:numValidClaws
    ...
    % Skip claw if there are fewer than 3 slices
    if numSlices < 3
        fprintf('  Warning: Claw %s has only %d slices, skipping...\n', clawNames{clawIdx}, numSlices);
        continue;
    end

    % Pre-allocate sample containers
    numSamples = numSlices - windowSize + 1;
    X = cell(numSamples, 1);
    Y = cell(numSamples, 1);
    count = 1;

    fprintf('  Creating 2.5D samples...\n');

    % Slide the 3-slice window across the volume
    for i = (1+halfWin):(numSlices-halfWin)
        try
            % Extract 3 consecutive slices
            s1 = volume(:,:,i-1);
            s2 = volume(:,:,i);
            s3 = volume(:,:,i+1);

            % Concatenate along channels => 3-channel input
            stack = cat(3, s1, s2, s3);
            ...
            % Resize the 3-channel stack to Inception-ResNet-v2 input size
            stack = imresize(stack, [networkSize, networkSize]);

            % Store the 3-channel 2.5D stack
            X{count} = stack;

            % Label = center slice of the window
...
fprintf('\n========================================');
fprintf('\nSTARTING LOOCV WITH VALIDATION SET (2.5D DEEPLABV3+ INCEPTION-RESNET-V2)');
fprintf('\n========================================\n');
...
for testClawIdx = 1:numValidClaws
    ...
    % ================================================================
    %  BUILD DEEPLABV3+ WITH INCEPTION-RESNET-V2 BACKBONE (2.5D INPUT)
    %  ----------------------------------------------------------------
    %  MATLAB's deeplabv3plusLayers SUPPORTS 'inceptionresnetv2' as a
    %  base network option. This is a native call that automatically
    %  constructs the encoder-decoder architecture with ASPP and skip
    %  connections.
    %
    %  Input shape: [299 299 3]
    %  Note: the 3 channels are the 2.5D stack (prev/center/next slice).
    %        Inception-ResNet-v2 consumes this as a 3-channel tensor.
    %
    %  IMPORTANT: The network input size must be [299 299 3] to match
    %             Inception-ResNet-v2's native input dimensions.
    % ================================================================

    % DeepLabv3+ input image size (Inception-ResNet-v2 native size)
    imageSize = [299, 299, 3];

    try
        % Construct DeepLabv3+ with Inception-ResNet-v2 backbone
        lgraph = deeplabv3plusLayers(imageSize, numClasses, 'inceptionresnetv2');
        fprintf('Using 2.5D DeepLabv3+ with Inception-ResNet-v2 backbone\n');
    catch ME
        % If deeplabv3plusLayers fails, report the error clearly
        fprintf('Error creating DeepLabv3+: %s\n', ME.message);
        fprintf('Ensure Computer Vision Toolbox and Inception-ResNet-v2 support package are installed.\n');
        rethrow(ME);
    end
    ...
    options = trainingOptions("adam", ...                  % Adam optimizer
        ...
        "MiniBatchSize", 4, ...                            % Small batch (Inception-ResNet-v2 is heavy)
    ...
    % -------- Train Network --------
    fprintf('Training Fold %d (2.5D DeepLabv3+ Inception-ResNet-v2)...\n', testClawIdx);
    ...
    save(['deeplabv3plus_irv2_25d_model_fold_' num2str(testClawIdx) '.mat'], 'net');
    fprintf('OK 2.5D DeepLabv3+ (Inception-ResNet-v2) model saved as: deeplabv3plus_irv2_25d_model_fold_%d.mat\n', testClawIdx);
    ...
    fprintf('\nOK Fold %d 2.5D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy: %.2f%%\n', testClawIdx, accuracy * 100);
...
%% ------------------ 6. OVERALL RESULTS -----------------------
fprintf('\n========================================');
fprintf('\nLEAVE-ONE-OUT CROSS-VALIDATION RESULTS (2.5D DEEPLABV3+ INCEPTION-RESNET-V2)');
fprintf('\n========================================\n');
...
sgtitle('LOOCV 2.5D DeepLabv3+ (Inception-ResNet-v2) Results for All Claws');
...
title('LOOCV 2.5D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy per Claw');
ylim([0 100]);
grid on;
...
ylabel('Test Accuracy (%)');
title('LOOCV 2.5D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy Distribution');
grid on;
...
results.networkType           = '2.5D DeepLabv3+ (Inception-ResNet-v2 backbone, 3-slice stack)';

% Save results to disk
save('LOOCV_deeplabv3plus_irv2_25d_results_with_validation.mat', 'results');
fprintf('\nOK Results saved to LOOCV_deeplabv3plus_irv2_25d_results_with_validation.mat\n');

fprintf('\n========================================');
fprintf('\n2.5D DEEPLABV3+ (INCEPTION-RESNET-V2) PROCESS COMPLETED SUCCESSFULLY!');
fprintf('\n========================================\n');
