%% ========================================================================
%  CLAW SEGMENTATION WITH DEEPLABV3+ (Inception-ResNet-v2 Backbone, 2D)
%  ------------------------------------------------------------------------
...
for clawIdx = 1:numValidClaws
    ...
    for i = 1:numSlices
        try
            ...
            % ---- Replicate grayscale to 3 channels for Inception-ResNet-v2 ----
...
fprintf('\n========================================');
fprintf('\nSTARTING LOOCV WITH VALIDATION SET (2D DEEPLABV3+ INCEPTION-RESNET-V2)');
fprintf('\n========================================\n');
...
for testClawIdx = 1:numValidClaws
    ...
    % ================================================================
    %  BUILD DEEPLABV3+ WITH INCEPTION-RESNET-V2 BACKBONE (2D INPUT)
    %  ----------------------------------------------------------------
    %  MATLAB's deeplabv3plusLayers natively supports 'inceptionresnetv2'
    %  as a base network option. This is a single function call that
    %  constructs the entire encoder-decoder + ASPP architecture.
    %
    %  Input shape: [299 299 3]
    %  Note: the 3 channels come from replicating a single grayscale
    %        slice, so the network sees the same slice in all three
    %        channels (no through-slice context, unlike 2.5D).
    % ================================================================
    ...
    try
        ...
        fprintf('Using 2D DeepLabv3+ with Inception-ResNet-v2 backbone\n');
    catch ME
    ...
    % -------- Train Network --------
    fprintf('Training Fold %d (2D DeepLabv3+ Inception-ResNet-v2)...\n', testClawIdx);
    ...
    save(['deeplabv3plus_irv2_model_fold_' num2str(testClawIdx) '.mat'], 'net');
    fprintf('OK 2D DeepLabv3+ (Inception-ResNet-v2) model saved as: deeplabv3plus_irv2_model_fold_%d.mat\n', testClawIdx);
    ...
    fprintf('\nOK Fold %d 2D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy: %.2f%%\n', testClawIdx, accuracy * 100);
...
%% ------------------ 6. OVERALL RESULTS -----------------------
fprintf('\n========================================');
fprintf('\nLEAVE-ONE-OUT CROSS-VALIDATION RESULTS (2D DEEPLABV3+ INCEPTION-RESNET-V2)');
fprintf('\n========================================\n');
...
sgtitle('LOOCV 2D DeepLabv3+ (Inception-ResNet-v2) Results for All Claws');
...
title('LOOCV 2D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy per Claw');
ylim([0 100]);
grid on;
...
ylabel('Test Accuracy (%)');
title('LOOCV 2D DeepLabv3+ (Inception-ResNet-v2) Test Accuracy Distribution');
grid on;
...
results.networkType           = '2D DeepLabv3+ (Inception-ResNet-v2 backbone, single-slice)';

% Save results to disk
save('LOOCV_deeplabv3plus_irv2_results_with_validation.mat', 'results');
fprintf('\nOK Results saved to LOOCV_deeplabv3plus_irv2_results_with_validation.mat\n');

fprintf('\n========================================');
fprintf('\n2D DEEPLABV3+ (INCEPTION-RESNET-V2) PROCESS COMPLETED SUCCESSFULLY!');
fprintf('\n========================================\n');
