%% ========================================================================
%  CLAW SEGMENTATION WITH DEEPLABV3+ (ResNet50 Backbone, 2.5D Input)
%  ------------------------------------------------------------------------
...
for clawIdx = 1:numValidClaws
    ...
    for i = (1+halfWin):(numSlices-halfWin)
        try
            ...
            % Resize the 3-channel stack to network input size
            stack = imresize(stack, [networkSize, networkSize]);

            % Store the 3-channel 2.5D stack (already 3-channel, no replication)
            X{count} = stack;
...
fprintf('\n========================================');
fprintf('\nSTARTING LOOCV WITH VALIDATION SET (2.5D DEEPLABV3+ RESNET50)');
fprintf('\n========================================\n');
...
for testClawIdx = 1:numValidClaws
    ...
    % ================================================================
    %  BUILD DEEPLABV3+ WITH RESNET50 BACKBONE (2.5D INPUT)
    %  ----------------------------------------------------------------
    ...
    %  - Input shape: [224 224 3] (2.5D stack: 3 consecutive slices)
    %  - Output: per-pixel class scores + softmax + pixelClassification
    %
    %  Note: The 3-channel input is the 2.5D stack (prev/center/next
    %        slice), NOT an RGB image. ResNet50 simply consumes it as a
    %        3-channel tensor, which is exactly what a 2.5D CNN needs.
    % ================================================================

    % DeepLabv3+ input image size (3-channel 2.5D stack)
    imageSize = [224, 224, 3];

    try
        % Construct DeepLabv3+ with ResNet50 backbone
        lgraph = deeplabv3plusLayers(imageSize, numClasses, 'resnet50');
        fprintf('Using 2.5D DeepLabv3+ with ResNet50 backbone\n');
    catch ME
    ...
    % -------- Train Network --------
    fprintf('Training Fold %d (2.5D DeepLabv3+ ResNet50)...\n', testClawIdx);
    ...
    save(['deeplabv3plus_25d_model_fold_' num2str(testClawIdx) '.mat'], 'net');
    fprintf('OK 2.5D DeepLabv3+ model saved as: deeplabv3plus_25d_model_fold_%d.mat\n', testClawIdx);
    ...
    fprintf('\nOK Fold %d 2.5D DeepLabv3+ Test Accuracy: %.2f%%\n', testClawIdx, accuracy * 100);
...
%% ------------------ 6. OVERALL RESULTS -----------------------
fprintf('\n========================================');
fprintf('\nLEAVE-ONE-OUT CROSS-VALIDATION RESULTS (2.5D DEEPLABV3+ RESNET50)');
fprintf('\n========================================\n');
...
sgtitle('LOOCV 2.5D DeepLabv3+ (ResNet50) Results for All Claws');
...
title('LOOCV 2.5D DeepLabv3+ (ResNet50) Test Accuracy per Claw');
ylim([0 100]);
grid on;
...
ylabel('Test Accuracy (%)');
title('LOOCV 2.5D DeepLabv3+ (ResNet50) Test Accuracy Distribution');
grid on;
...
results.networkType           = '2.5D DeepLabv3+ (ResNet50 backbone, 3-slice stack)';

% Save results to disk
save('LOOCV_deeplabv3plus_25d_results_with_validation.mat', 'results');
fprintf('\nOK Results saved to LOOCV_deeplabv3plus_25d_results_with_validation.mat\n');

fprintf('\n========================================');
fprintf('\n2.5D DEEPLABV3+ (RESNET50) PROCESS COMPLETED SUCCESSFULLY!');
fprintf('\n========================================\n');
...
% ---- loadLabelFromFile ----
% Custom reader for pixelLabelDatastore that loads a .mat file and
% returns the variable 'label' as the pixel label map.
