%% ========================================================================
%  CLAW SEGMENTATION WITH DEEPLABV3+ (ResNet101 Backbone, 2.5D Input)
%  ------------------------------------------------------------------------
...
for clawIdx = 1:numValidClaws
    ...
    for i = (1+halfWin):(numSlices-halfWin)
        try
            ...
            % Store the 3-channel 2.5D stack (no replication needed)
            X{count} = stack;
...
fprintf('\n========================================');
fprintf('\nSTARTING LOOCV WITH VALIDATION SET (2.5D DEEPLABV3+ RESNET101)');
fprintf('\n========================================\n');
...
for testClawIdx = 1:numValidClaws
    ...
    % ================================================================
    %  BUILD DEEPLABV3+ WITH RESNET101 BACKBONE (2.5D INPUT)
    %  ----------------------------------------------------------------
    ...
    %        not RGB. ResNet101 simply consumes it as a 3-channel tensor.
    % ================================================================
    ...
    try
        ...
        fprintf('Using 2.5D DeepLabv3+ with ResNet101 backbone (manually built)\n');
    ...
    % -------- Train Network --------
    fprintf('Training Fold %d (2.5D DeepLabv3+ ResNet101)...\n', testClawIdx);
    ...
    save(['deeplabv3plus_r101_25d_model_fold_' num2str(testClawIdx) '.mat'], 'net');
    fprintf('OK 2.5D DeepLabv3+ (ResNet101) model saved as: deeplabv3plus_r101_25d_model_fold_%d.mat\n', testClawIdx);
    ...
    fprintf('\nOK Fold %d 2.5D DeepLabv3+ (ResNet101) Test Accuracy: %.2f%%\n', testClawIdx, accuracy * 100);
...
%% ------------------ 6. OVERALL RESULTS -----------------------
fprintf('\n========================================');
fprintf('\nLEAVE-ONE-OUT CROSS-VALIDATION RESULTS (2.5D DEEPLABV3+ RESNET101)');
fprintf('\n========================================\n');
...
sgtitle('LOOCV 2.5D DeepLabv3+ (ResNet101) Results for All Claws');
...
title('LOOCV 2.5D DeepLabv3+ (ResNet101) Test Accuracy per Claw');
ylim([0 100]);
grid on;
...
ylabel('Test Accuracy (%)');
title('LOOCV 2.5D DeepLabv3+ (ResNet101) Test Accuracy Distribution');
grid on;
...
results.networkType           = '2.5D DeepLabv3+ (ResNet101 backbone, 3-slice stack)';

% Save results to disk
save('LOOCV_deeplabv3plus_r101_25d_results_with_validation.mat', 'results');
fprintf('\nOK Results saved to LOOCV_deeplabv3plus_r101_25d_results_with_validation.mat\n');

fprintf('\n========================================');
fprintf('\n2.5D DEEPLABV3+ (RESNET101) PROCESS COMPLETED SUCCESSFULLY!');
fprintf('\n========================================\n');